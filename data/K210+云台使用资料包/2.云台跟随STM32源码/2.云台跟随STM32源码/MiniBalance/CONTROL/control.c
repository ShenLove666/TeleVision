...
uint8_t send_k210_flag = 0;//发送数据到k210的标志位
uint8_t seltec_vaule = 0;//颜色选择
#define MAX_COLOR 4 //支持的颜色数量
...
{	
    ...
    double error = pid->setpoint - input;//本次偏差
	double derivative = (input - pid->last_input);//上一次偏差
...
int TIM2_IRQHandler(void)
{
    if (TIM_GetITStatus(TIM2, TIM_IT_Update) != RESET) //检查指定的TIM中断发生与否:TIM 中断源
    {
		...
		ObjectTrack.count_Outline++;
		if (ObjectTrack.count_Outline>100) ObjectTrack.count_Outline=101,ObjectTrack.lost=1;
        ...
        if(Turn_Off(Voltage)==0)               //===如果电池电压不存在异常
        {
			...
			///// 物品跟随  /// 
			static u16 obj_timeout = 0;//检测物体识别超时行为
			if( ObjectTrack.lost==0 ) //识别物体才动作
			{	
				obj_timeout = 0;//复位变量,若存在识别超时才使用
				
				float tar1;
				tar1 = PID_Update(&FollowPID_X,ObjectTrack.cam_centerX,ObjectTrack.centerX);
				Target1+=tar1;
				
				float tar2;
				tar2 = PID_Update(&FollowPID_Y,ObjectTrack.cam_centerY,ObjectTrack.centerY);
				Target2+=tar2;				
...
void Xianfu_Pwm(void)
{
    ...
    int Amplitude_H1=1200, Amplitude_L1=300;  //中心位置为750 ，运动范围±450  750±450；  750-250）/90=5.5；    5.5*80≈450；
...
//    if(temp==1)	Mode_Usart_PS2=!Mode_Usart_PS2;//单击切换PS2手柄/Usart串口控制模式
	if( temp==1 )
	{
		seltec_vaule++;//选择颜色
		send_k210_flag = 1;
		if( seltec_vaule > MAX_COLOR-1 ) seltec_vaule = 0;
	}
	
	//长按复位
	u8 long_p = Long_Press();
	if( long_p==1 )
	{
		Target1 = 750;
		Target2 = 750;
